...
```{r}
#read data
rating<-read.csv("rating_time.csv",header = TRUE)
head(rating)
#rating$id<-c(1:100836)
#sort data based on time
summary(rating$time_bin)
rating_sort<-rating[order(rating$time_bin),]
...
index1<-duplicated(rating_sort[,2])
#extract index of unique movie
index2<-duplicated(rating_sort[,6])
#extract index of unique timebin
index3<-duplicated(rating_sort[,8])
